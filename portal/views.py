...
def dashboard_view(request):
    ...
    for course in student_courses:
        ...
        unlocked_playlist_ids = []
        
        for playlist in playlists:
            is_unlocked = PlaylistAccessCode.objects.filter(
                playlist=playlist, 
                user=user, 
                is_used=True
            ).exists()
...
def unlock_playlist_view(request, playlist_id):
    ...
    if PlaylistAccessCode.objects.filter(user=request.user, playlist=playlist, is_used=True).exists():
...
def playlist_lessons_view(request, playlist_id):
    ...
    if not PlaylistAccessCode.objects.filter(user=request.user, playlist=playlist, is_used=True).exists():
